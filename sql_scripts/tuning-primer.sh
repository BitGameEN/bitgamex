...
## -- Check for the existance of plesk and login using it's credentials -- ##
	...
	if [ -f /etc/psa/.psa.shadow ] ; then
	        mysql="/usr/local/mysql/bin/mysql -S $socket -u admin -p$(cat /etc/psa/.psa.shadow)"
	        mysqladmin="/usr/local/mysql/bin/mysqladmin -S $socket -u admin -p$(cat /etc/psa/.psa.shadow)"
	else
