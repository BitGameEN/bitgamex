...
init() ->
	ets:new(run_role_gold, [{keypos, #run_role_gold.key_id}, named_table, public, set, ?ETSRC, ?ETSWC]),
	ok.
...
get_one({Game_id, Player_id} = Id) ->
	case run_data:in_trans() of
		true ->
			case run_data:trans_get(run_role_gold, Id) of
				[] ->
					get_one(Id, anyway);
				trans_deleted -> [];
...
get_one({Game_id, Player_id} = Id, just_from_ets) ->
	run_data:lookup_one(run_role_gold, Id, false);
get_one({Game_id, Player_id} = Id, direct_from_db) ->
	case cache:get(cache_key(Id)) of
		{true, _Cas, Val0} ->
			...
			Val;
		_ ->
			case db_esql:get_row(?DB_RUN, io_lib:format(<<"select player_id,ver,game_id,gold,time from role_gold_~p where game_id=? and player_id=?">>, [Game_id]), [Game_id, Player_id]) of
				[] -> [];
				Row ->
...
get_one({Game_id, Player_id} = Id, anyway) ->
	case run_data:lookup_one(run_role_gold, Id, false) of
		[] ->
			case cache:get(cache_key(Id)) of
				{true, _Cas, Val0} ->
					Val = run_data_ver:upgrade_if_need(Val0),
...
set_field({Game_id, Player_id} = Id, Field, Val) ->
	Fields = record_info(fields, run_role_gold),
...
set_one(R0) when is_record(R0, run_role_gold) ->
	case R0#run_role_gold.key_id =:= undefined of
		false ->
			case run_data:in_trans() of
				true ->
					run_data:trans_set(run_role_gold, R0#run_role_gold.key_id, R0,
						void,
						void);
				false ->
					case util:lookup_one(run_role_gold, R0#run_role_gold.key_id, false) of
						[] ->
					...
					syncdb(R0),
					cache:set(cache_key(R0#run_role_gold.key_id), R0)
			end,
			R0#run_role_gold.key_id;
		true ->
			#run_role_gold{
				player_id = Player_id,
				ver = Ver,
			...
			case run_data:in_trans() of
				true ->
					run_data:trans_set(run_role_gold, R#run_role_gold.key_id, {trans_inserted, R}, F, fun() -> run_role_gold:del_one(R, true) end);
				false ->
					F()
			end,
			R#run_role_gold.key_id
	end.

del_one(R) when is_record(R, run_role_gold) ->
	del_one(R, false);
del_one({Game_id, Player_id} = Id) ->
	del_one(Id, false).

del_one(R, DelDbAlso) when is_record(R, run_role_gold) ->
	case run_data:in_trans() of
		true ->
			run_data:trans_del(run_role_gold, R#run_role_gold.key_id,
				fun() -> run_role_gold:del_one(R, DelDbAlso) end,
				void);
		false ->
			{Game_id, Player_id} = R#run_role_gold.key_id,
			case DelDbAlso of
				true ->
					run_data:db_write(del, R, fun() -> db_esql:execute(?DB_RUN, io_lib:format(<<"delete from role_gold_~p where game_id=? and player_id=?">>, [Game_id]), [Game_id, Player_id]) end),
					cache:del(cache_key(R#run_role_gold.key_id));
				false -> void
			end,
			ets:delete(run_role_gold, R#run_role_gold.key_id)
	end,
	ok;
del_one({Game_id, Player_id} = Id, DelDbAlso) ->
	case util:lookup_one(run_role_gold, Id, false) of
		[] ->
			case DelDbAlso of
				true ->
...
clean_all_cache(N, Game_id) ->
	case db_esql:get_all(?DB_RUN, io_lib:format(<<"select game_id, player_id from role_gold_~p limit ?, 1000">>, [Game_id]), [N * 1000]) of
		[] -> ok;
		Rows ->
...
syncdb(R) when is_record(R, run_role_gold) ->
	#run_role_gold{
		player_id = Player_id,
		ver = Ver,
		game_id = Game_id,
...
cache_key({Game_id, Player_id} = Id) ->
	list_to_binary(io_lib:format(<<"run_role_gold_~p_~p">>, [Game_id, Player_id])).
