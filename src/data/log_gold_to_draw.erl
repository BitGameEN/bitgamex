...
set_one(R0) when is_record(R0, log_gold_to_draw) ->
	case R0#log_gold_to_draw.key_id =:= undefined of
		...
		true ->
			...
			{ok, [[Insert_id|_]]} = db_esql:multi_execute(?DB_LOG, io_lib:format(<<"insert into gold_to_draw(id,game_id,player_id,gold_type,delta,old_value,new_value,drain_type,drain_id,drain_count,time,call_flow) values(~p,~p,~p,'~s',~p,~p,~p,'~s',~p,~p,~p,'~s'); select last_insert_id()">>,
