...
set_one(R0) when is_record(R0, log_gold_on_user) ->
	case R0#log_gold_on_user.key_id =:= undefined of
		...
		true ->
			...
			{ok, [[Insert_id|_]]} = db_esql:multi_execute(?DB_LOG, io_lib:format(<<"insert into gold_on_user(id,game_id,player_id,gold_type,delta,old_value,new_value,drain_type,drain_id,drain_count,time,call_flow) values(~p,~p,~p,'~s',~p,~p,~p,'~s','~s',~p,~p,'~s'); select last_insert_id()">>,
...
syncdb(R) when is_record(R, log_gold_on_user) ->
	...
	db_esql:execute(?DB_LOG, <<"replace into gold_on_user(id,game_id,player_id,gold_type,delta,old_value,new_value,drain_type,drain_id,drain_count,time,call_flow) values(?,?,?,?,?,?,?,?,?,?,?,?)">>,
		[Id, Game_id, Player_id, Gold_type, Delta, Old_value, New_value, Drain_type, Drain_id, Drain_count, Time, Call_flow]).
