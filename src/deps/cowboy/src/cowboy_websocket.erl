...
upgrade(Req, Env, Handler, HandlerState, Timeout, Hibernate) ->
	...
	try websocket_upgrade(State, Req) of
        ...
        lager:error("websocket upgrade err=~p~nreq=~p~n", [Err, Req]),
...
handler_loop(State=#state{socket=Socket, messages={OK, Closed, Error},
	...
	receive
		{OK, Socket, Data} ->
            check_memory(State, Req, HandlerState),
...
check_memory(#state{socket = Socket, transport = Transport} = State, Req, HandlerState) ->
    case Transport of
        ranch_ssl ->
            Limit = 1024 * 1024,
            Pid = Socket#sslsocket.pid,
            case util:get_memory(Pid) > Limit orelse
                     util:get_bin_memory(Pid) > 2 * Limit of
                true ->
                    ?ERR("tls_connection memory over 1M~n", []),
                    supervisor:terminate_child(tls_connection_sup, Pid),
                    handler_terminate(State, Req, HandlerState, {error, memory_limit_reached});
                false -> void
            end;
        _ -> void
    end.
