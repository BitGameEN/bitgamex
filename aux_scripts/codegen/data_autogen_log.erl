...
gen_erl(Table, ErlName, ModuleName, FieldNames, FieldTypes, FieldKeyInfos, RecordName) ->
    ...
    io:format(S, "\t\t\t} = R0,~n", []),
    io:format(S, "\t\t\tspawn(fun() -> {ok, [[Insert_id|_]]} = db_esql:multi_execute(?DB_LOG, io_lib:format(<<\"insert into ~s(~s) values(~s); select last_insert_id()\">>,~n\t\t\t\t[~s])) end)~n",
    ...
    gen_erl_fields(FieldNames, FieldNames2, S, <<"\t\t">>),
    io:format(S, "\t} = R,~n", []),
    io:format(S, "\tspawn(fun() -> db_esql:execute(?DB_LOG, <<\"replace into ~s(~s) values(~s)\">>,~n\t\t[~s]) end).~n~n",
